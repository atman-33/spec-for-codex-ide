...
- Recognize **Spec for Codex IDE** as a VS Code extension that orchestrates spec-driven workflows for Codex CLI users; always keep the focus on managing `.codex/specs`, `.codex/steering`, and `.codex/prompts` assets surfaced through the tree views (`src/providers/*-explorer-provider.ts`).
